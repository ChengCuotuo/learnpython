...
func:爬取qq说说数据
使用步骤：
1.安装谷歌浏览器； 适用的 chromedriver.exe 就是Google浏览器的启动器
2.安装驱动chromedriver.exe，并把它放在程序目录下；
3修改user和pw为你自己的号码和密码；4在user.txt中
写上你想爬取的好友qq号码，一行一个号码
'''
# 安装谷歌浏览器，然后安装驱动
import time
...
def qq_zone(friend):
    try:
        user = '1433893622'  # 1.你的QQ号
        pw = 'ChunLei@1105'  # 2.你的QQ密码
        #获取浏览器驱动
        driver = webdriver.Chrome(executable_path='chromedriver')
        # 浏览器窗口最大化
        driver.set_window_size(400,400)#设定固定大小
        # 浏览器地址定向为qq登陆页面
        ...
        while True:
                ...
                f = open(friend+'.txt','a',encoding='utf-8')
                for div in divs:
        ...
        driver.close()
        return None
with open('user.txt','r') as f:#3.修改qq号4.驱动放在运行目录下
    cath = f.read().split('\n')
i=0
for each in cath:
    i += 1
